...
extension HomeView {
    final class HomeViewModel: ObservableObject {
        ...
        @Published var showDeleteEventAlert = false
        var eventIdToDelete: String?
        ...
        let motivationItems: [MotivationItem] = [
            ...
            .init(image: Asset.motivation2.name,
                  title: "Podróżuj po świecie bez ekranu",
            ...
            .init(image: Asset.motivation3.name,
                  title: "Poszukiwanie nowych doświadczeń",
        ...
        func deleteEvent() {
            DispatchQueue.main.async {
                guard let id = self.eventIdToDelete else { return}
                DefaultsService.removeEventWith(eventId: id)
                self.setEvents()
                self.eventIdToDelete = nil
            }
        }
    }
}
